"""
orfs_config_gen.py
Auto-generates ORFS config.mk from a validated eSim .cir.out netlist.
"""
...
def generate_config(cir_out_path, design_name=None, pdk="sky130",
    ...
    if report["summary"] == "FAIL":
        raise RuntimeError(
            f"Validation FAILED - {len(report['unmappable'])} unmappable primitive(s). "
            f"Fix the netlist before generating ORFS config."
        )

    if design_name is None:
        stem = Path(cir_out_path).stem
        design_name = stem.replace(".cir", "").replace(".", "_")
    ...
    lines = [
        ...
        f"# Source netlist : {Path(cir_out_path).resolve()}",
        f"# Generated      : {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
        f"# PDK            : {pdk}",
        f"",
        f"export DESIGN_NAME       = {design_name}",
        f"export PLATFORM          = {defaults['PLATFORM']}",
        f"",
        ...
        f"export CORE_MARGIN       = {defaults['CORE_MARGIN']}",
        f"",
        f"export CLOCK_PORT        = {clock_port}",
        f"export CLOCK_PERIOD      = {clk_period}",
        f"",
        f"export TARGET_DENSITY    = {defaults['TARGET_DENSITY']}",
        f"",
    ...
    if report["analog_primitives"]:
        lines.append(f"# Skipped analog primitives ({len(report['analog_primitives'])}):")
...
if __name__ == "__main__":
    ...
    try:
        generate_config(args.netlist, args.design, args.pdk,
                        args.clock_port, args.clock_period, args.output_dir)
